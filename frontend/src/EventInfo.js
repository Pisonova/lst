...
export default function EventInfo(props) {
    ...
    const HandleProgramReg = (id) =>{
        axios.post(`/api/register_program`, {
    ...
    const getData =async ()=> {
        try {
            const {data} = await axios.get(`/api/programs/${id}`, { params:
                {token: localStorage["token"]}
            })
            setEvent(data[0])
        } catch (error) {
    ...
    if (event?.programs != null) {
        ...
        {item.more_info !== null &&
            ...
            {item.registered && org && <Button href={`/registered/program/${item.id}`}> Prihlásení používatelia </Button>}
    ...
    return (<>
        ...
        {event.more_info != null && event !== "" && 
            ...
            {event.registered && org && <Button href={`/registered/event/${event.id}`}> Prihlásení používatelia </Button>}
            {event.registered && <div className='logOut'><Button color='inherit' onClick={() => HandleELogOut(event)}> Zrušiť registráciu </Button></div>}
                </div>}
            {myList}
            {event != null && org && event.registered && 
